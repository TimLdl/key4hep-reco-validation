...
while IFS=$'\t' read -r detector version _slug validation config_path _config_dir _config_rel_dir _particle _output_tag _energy _seed _sim_script _hist_script; do
    [[ -z "$detector" ]] && continue
    ((selected_count += 1))
